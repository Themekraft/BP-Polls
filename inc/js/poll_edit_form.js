jQuery(document).ready(function() {
	...
	jQuery('.show_all_cats').click(function(){
		jQuery(this).hide();
		jQuery('.cat').show();
	});
...
function add_category(){
	var cat_str = jQuery('.category_field').val().trim();
	...
	for(var i in cats ) {
		...
		if( cat != '' ) {
			...
			jQuery('.categories .cat .name').each(function(){
				if( cat.toLowerCase() == jQuery(this).text().toLowerCase() ) {
			...
			if( cat_found == 0) {
				jQuery('.categories').prepend( jQuery('.cat_prototype').html().replace('CATNAME','cats[0#'+cat+']').replace('CAT_NAME',cat ) );
			}
		}
	}
...
function add_tag(){
	var tags_str = jQuery('.tag_field').val().trim();
	...
	for(var i in tags ) {
		...
		jQuery('.tags .tag .tag_name').each(function(){
			if( tag.toLowerCase() == jQuery(this).text().toLowerCase() ) {
				tag_found = 1;
			}
		});
