...
<?php if( vpl_is_user_can_modify_polls() ):?>
	<div class="vpl-poll-actions">
		<a class="button" href="<?php echo VPL_CURRENT_COMPONENT_URL . 'taxonomy/new/?type=tag'?>" /><?php _e('Add Tag', 'bp_polls')?></a>
	</div>
<?php endif;?>
    ...
    <ul class="vpl-categories-list">
		<?php foreach($vpl_group_tags as $tag):?>
			<li>
				...
				<?php if( vpl_is_user_can_modify_polls() ):?>
					<div class="options">
						<a href="<?php echo VPL_CURRENT_COMPONENT_URL . 'taxonomy/edit/'.$tag->id ?>" ><?php _e('Edit', 'bp_polls')?></a> | <a class="delete-category" href="javascript:void(0)" ><?php _e('Delete', 'bp_polls')?></a>
						<form class="hidden" method="post" action="">
